...
function saveHosts() {
  var data = Array.from(hosts.values()).map(function(h) {
    return { name:h.name, addr:h.addr, probeType:h.probeType, port:h.port, interval:h.interval, warnMs:h.warnMs, paused:h.paused||false, filial:h.filial||'', _placeholder:h._placeholder||false };
  });
...
function doPing(addr,tout){tout=tout||5000;return new Promise(function(ok){var sec=Math.ceil(tout/1000);var cmd=isWin?'ping -n 1 -w '+tout+' '+addr:isMac?'ping -c 1 -W '+sec+' -t '+sec+' '+addr:'ping -c 1 -W '+sec+' '+addr;var t0=Date.now();exec(cmd,{timeout:tout+3000},function(err,out,stderr){var el=Date.now()-t0;var stdout=(out||'')+(stderr||'');if(!out||!out.trim()){ok({alive:false,ms:0,out:'Request timed out.'});return;}var low=out.toLowerCase();var dead=low.indexOf('100% packet loss')>=0||low.indexOf('100% loss')>=0||low.indexOf('100%')>=0||low.indexOf('100 %')>=0||low.indexOf('unreachable')>=0||low.indexOf('timed out')>=0||low.indexOf('tempo limite')>=0||low.indexOf('solicitação expirou')>=0||low.indexOf('soli\u00e7a\u00e3o expirou')>=0||low.indexOf('recebidos = 0')>=0||low.indexOf('received = 0')>=0||low.indexOf('request timed out')>=0;if(dead){ok({alive:false,ms:0,out:out.trim().split('\n').pop()||'Request timed out.'});return;}var m=out.match(/[Tt]ime[<=](\d+\.?\d*)\s*ms/)||out.match(/tempo[<=](\d+\.?\d*)\s*ms/i)||out.match(/(\d+\.?\d*)\s*ms/);var ms=m?Math.round(parseFloat(m[1])):el;ok({alive:true,ms:ms||el,out:out.trim().split('\n').pop()||''});});});}
...
function probe(host){
  if(host._placeholder)return;
  var p=host.probeType==='TCP'?doTcp(host.addr,host.port,host.timeout):host.probeType==='DNS'?doDns(host.addr):doPing(host.addr,host.timeout);
  p.then(function(r){var prev=host.status,next=(!r.alive||r.ms>host.warnMs)?'down':'up';host.ms=r.ms;host.status=next;host.lastOut=r.out;host.hist.push({ms:r.ms,ts:Date.now()});if(host.hist.length>300)host.hist.shift();host.sent++;if(!r.alive)host.lost++;if(r.alive)host.lastSeen=Date.now();if(prev!=='check'&&prev!==next){host.changes.push({from:prev,to:next,ts:Date.now()});if(host.changes.length>50)host.changes.shift();bcast({e:'alert',id:host.id,name:host.name,addr:host.addr,from:prev,to:next,ts:Date.now()});}bcast({e:'update',host:snap(host)});}).catch(function(e){host.status='down';host.ms=0;host.lastOut='Erro: '+e.message;host.hist.push({ms:0,ts:Date.now()});host.sent++;host.lost++;bcast({e:'update',host:snap(host)});});}
function snap(h){return{id:h.id,name:h.name,addr:h.addr,probeType:h.probeType,port:h.port,status:h.status,ms:h.ms,sent:h.sent,lost:h.lost,lastOut:h.lastOut,lastSeen:h.lastSeen,hist:h.hist.slice(-60),changes:h.changes.slice(-10),interval:h.interval,warnMs:h.warnMs,paused:h.paused||false,filial:h.filial||'',_placeholder:h._placeholder||false};}
function makeHost(d){return{id:nextId++,name:d.name||d.addr,addr:d.addr,probeType:d.probeType||'ICMP',port:parseInt(d.port)||80,status:'check',ms:0,sent:0,lost:0,lastOut:'',lastSeen:null,hist:[],changes:[],interval:parseInt(d.interval)||2000,timeout:parseInt(d.timeout)||2000,paused:d.paused||false,warnMs:parseInt(d.warnMs||d.warnThreshold)||150,filial:d.filial||'',_placeholder:d._placeholder||false,_t:null};}
function startLoop(h){if(h._t)clearInterval(h._t);if(h.paused||h._placeholder)return;probe(h);h._t=setInterval(function(){probe(h);},h.interval);}
...
// API: lista de filiais registradas
function getFiliais(){
  var set=new Set();
  hosts.forEach(function(h){ if(h.filial) set.add(h.filial); });
...
var srv=http.createServer(function(req,res){
  ...
  if(req.url==='/api/filiais'&&req.method==='GET'){res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify(getFiliais()));return;}
  if(req.url.startsWith('/api/hosts/')&&req.method==='DELETE'){var id=parseInt(req.url.split('/')[3]),h=hosts.get(id);if(h){clearInterval(h._t);hosts.delete(id);bcast({e:'removed',id:id});saveHosts();}res.writeHead(200,{'Content-Type':'application/json'});res.end('{"ok":true}');return;}
...
srv.on('upgrade',function(req,sock){
  ...
  sock.on('data',function(chunk){
    ...
    try{
      ...
      if(action==='add_host'){
        // Remove placeholder da filial se existir
        if(msg.filial){
          hosts.forEach(function(h){
            if(h._placeholder&&h.filial===msg.filial){clearInterval(h._t);hosts.delete(h.id);bcast({e:'removed',id:h.id});}
          });
        }
        var nh=makeHost(msg);hosts.set(nh.id,nh);startLoop(nh);bcast({e:'added',host:snap(nh)});saveHosts();
      }
      ...
      else if(action==='edit_host'){var eh=hosts.get(msg.id);if(eh){clearInterval(eh._t);eh.name=msg.name||eh.name;eh.addr=msg.addr||eh.addr;eh.probeType=msg.probeType||eh.probeType;eh.port=parseInt(msg.port)||eh.port;eh.filial=msg.filial!==undefined?msg.filial:eh.filial;eh._placeholder=false;eh.status='check';eh.ms=0;eh.hist=[];eh.sent=0;eh.lost=0;startLoop(eh);saveHosts();bcast({e:'edited',host:snap(eh)});}}
      ...
      else if(action==='add_filial'){
        // Adiciona uma nova filial com placeholder
        var nf=makeHost({name:'Adicionar hosts...',addr:'0.0.0.0',probeType:'ICMP',port:80,interval:2000,warnMs:150,paused:true,filial:msg.filial,_placeholder:true});
        hosts.set(nf.id,nf);
        bcast({e:'added',host:snap(nf)});
        saveHosts();
      }
...
if (saved && saved.length > 0) {
  ...
  var defs=[{addr:'8.8.8.8',name:'DNS Google',probeType:'ICMP',filial:'Parnaíba - PI'},{addr:'1.1.1.1',name:'DNS Cloudflare',probeType:'ICMP',filial:'Parnaíba - PI'},{addr:'google.com',name:'google.com',probeType:'DNS',filial:'Parnaíba - PI'},{addr:'google.com',name:'HTTPS google.com',probeType:'TCP',port:443,filial:'Parnaíba - PI'}];
  var ifaces=os.networkInterfaces();
  outer:for(var k in ifaces){for(var i=0;i<ifaces[k].length;i++){var iface=ifaces[k][i];if(iface.family==='IPv4'&&!iface.internal){defs.unshift({addr:iface.address.split('.').slice(0,3).join('.')+'.1',name:'Gateway Local',probeType:'ICMP',filial:'Parnaíba - PI'});break outer;}}}
